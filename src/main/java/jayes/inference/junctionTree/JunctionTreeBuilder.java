...
import jayes.util.triangulation.GraphElimination;
import jayes.util.triangulation.IEliminationHeuristic;
...
public class JunctionTreeBuilder {
    ...
    private List<Pair<Graph.Edge, List<Integer>>> computeSepsets(JunctionTree junctionTree, BayesNet net) {
        final List<Pair<Graph.Edge, List<Integer>>> candidates = enumerateCandidateSepSets(junctionTree.getClusters());
        Collections.sort(candidates, new SepsetComparator(net));
        return computeMaxSpanningTree(junctionTree.getGraph(), candidates);

    }

    private List<Pair<Graph.Edge, List<Integer>>> enumerateCandidateSepSets(List<List<Integer>> clusters) {
        final List<Pair<Graph.Edge, List<Integer>>> sepSets = new ArrayList<Pair<Graph.Edge, List<Integer>>>();
        ...
        while (it.hasNext()) {
            ...
            while (remainingIt.hasNext()) { // generate sepSets
                ...
                sepSets.add(newPair(new Graph.Edge(it.nextIndex() - 1, remainingIt.nextIndex() - 1), clique2));
            }
        }
        return sepSets;
    }

    private List<Pair<Graph.Edge, List<Integer>>> computeMaxSpanningTree(Graph graph,
                                                                         final List<Pair<Graph.Edge, List<Integer>>> sortedCandidateSepSets) {

        final ArrayDeque<Pair<Graph.Edge, List<Integer>>> pq = new ArrayDeque<Pair<Graph.Edge, List<Integer>>>(
        ...
        final List<Pair<Graph.Edge, List<Integer>>> leftSepSets = new ArrayList<Pair<Graph.Edge, List<Integer>>>();
        while (leftSepSets.size() < (vertexCount - 1)) {
            final Pair<Graph.Edge, List<Integer>> sep = pq.poll();
    ...
    private final class SepsetComparator implements Comparator<Pair<Graph.Edge, List<Integer>>> {
        ...
        @Override
        public int compare(final Pair<Graph.Edge, List<Integer>> sepSet1, final Pair<Graph.Edge, List<Integer>> sepSet2) {
