...
import jayes.factor.AbstractFactor;
import jayes.factor.arraywrapper.DoubleArrayWrapper;
import jayes.factor.arraywrapper.IArrayWrapper;
import jayes.inference.AbstractInferer;
import jayes.util.Graph;
import jayes.util.MathUtils;
import jayes.util.NumericalInstabilityException;
import jayes.util.Pair;
import jayes.util.sharing.CanonicalArrayWrapperManager;
import jayes.util.sharing.CanonicalIntArrayManager;
import jayes.util.triangulation.MinFillIn;
import util.ArrayUtils;

import java.util.*;

import static jayes.util.Pair.newPair;
...
public class JunctionTreeAlgorithm extends AbstractInferer {
    ...
    /**
     * checks which linkNodeParentChild need not be processed during collectEvidence (because of preprocessing). These are those linkNodeParentChild
     * without evidence which are leaves or which only have non-evidence descendants
     * 
     * @param root
     *            the node to start the check from
     * @return a set of the linkNodeParentChild not needing a call of collectEvidence
     */
    ...
    private void recursiveSkipCollection(final int node, final Set<Integer> visited, final Set<Integer> skipped) {
        ...
        for (final Graph.Edge e : junctionTree.getIncidentEdges(node)) {
    ...
    /**
     * checks which linkNodeParentChild do not need to be visited during evidence distribution. These are exactly those linkNodeParentChild which
     * are
     * <ul>
    ...
    private void collectEvidence(final int cluster, final Set<Integer> marked) {
        marked.add(cluster);
        for (final Graph.Edge e : junctionTree.getIncidentEdges(cluster)) {
    ...
    private void messagePass(final Graph.Edge sepSetEdge) {
    ...
    private boolean isOnlyFirstLogScale(final Graph.Edge edge) {
        return nodePotentials[edge.getFirst()].isLogScale() && !nodePotentials[edge.getSecond()].isLogScale();
    }

    private boolean isOnlySecondLogScale(final Graph.Edge edge) {
        return !nodePotentials[edge.getFirst()].isLogScale() && nodePotentials[edge.getSecond()].isLogScale();
    }
    ...
    private void initializeFields(int numNodes) {
        ...
        sepSets = new HashMap<Graph.Edge, AbstractFactor>();
    ...
    private void initializeSepsetFactors(final List<Pair<Graph.Edge, List<Integer>>> sepSets) {
        for (final Pair<Graph.Edge, List<Integer>> sep : sepSets) {
    ...
    private void initializePotentialValues() {
        ...
        for (final Map.Entry<Graph.Edge, AbstractFactor> sepSet : sepSets.entrySet()) {
    ...
    private boolean areBothEndsLogScale(final Graph.Edge edge) {
        return nodePotentials[edge.getFirst()].isLogScale() && nodePotentials[edge.getSecond()].isLogScale();
    }
